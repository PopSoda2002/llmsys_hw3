...
class MultiHeadAttention(Module):
    ...
    def project_to_query_key_value(self, x):
        ...
        q = self.q_projection(x_flat).view(
            batch_size, seq_len, self.n_head, self.attn_hidden_dim
        )
        k = self.k_projection(x_flat).view(
            batch_size, seq_len, self.n_head, self.attn_hidden_dim
        )
        v = self.v_projection(x_flat).view(
            batch_size, seq_len, self.n_head, self.attn_hidden_dim
        )
    ...
    def self_attention(self, q, kT, v):
        ...
        out = attn @ v
        result = out.permute(0, 2, 1, 3).contiguous().view(
            batch_size, queries_len, num_head * v_dim
        )
        ### END ASSIGN3_3
    ...
    def forward(self, x):
        ...
        out = self.out_projection(attn_out.view(batch_size * seq_len, n_embd))
...
class TransformerLayer(Module):
    def __init__(self, n_embd: int, n_head: int, p_dropout: float=0.1, ln_eps: float=1e-5, bias: bool=True, backend: TensorBackend=None):
        ...
        self.attention = MultiHeadAttention(
            n_embd, n_head, causal=True, p_dropout=p_dropout, bias=bias, backend=backend
        )
    ...
    def forward(self, x):
        ...
        batch_size, seq_len, n_embd = x.shape
        ### BEGIN YOUR SOLUTION
        ln1 = self.ln_1(x.view(batch_size * seq_len, n_embd)).view(
            batch_size, seq_len, n_embd
        )
        x = x + self.attention(ln1)
        ln2 = self.ln_2(x.view(batch_size * seq_len, n_embd)).view(
            batch_size, seq_len, n_embd
        )
        x = x + self.ff(ln2)
        return x
...
class DecoderLM(Module):
    ...
    def forward(self, idx):
        ...
        pos_ids = tensor_from_numpy(
            np.arange(seq_len).reshape(1, seq_len).astype(np.float32),
